...
0x00,0x20,0xC4,0xC8,0x38,0x08,0x08,0x08,0xF8,0x08,0x08,0x08,0x08,0x00,0x00,0x00,
0x7E,0x21,0x20,0x20,0x20,0x20,0x3F,0x20,0x20,0x20,0x20,0x00,/*"江",0*/

0x00,0x08,0x88,0x88,0x88,0x9E,0xE8,0x88,0xBE,0x88,0x88,0x08,0x08,0x00,0x00,0x88,
0x4C,0x20,0x10,0x0E,0x01,0x40,0x40,0x3F,0x00,0x02,0x0C,0x00,/*"苏",1*/
...
0x04,0x04,0x04,0x44,0x44,0x3E,0x05,0x04,0x04,0x04,0x04,0x00,/*"学",3*/
};
...
/*
 *函数名:LED_WrDat(uint8 data)
 *功能  :写数据函数,使得oled屏幕上8个点亮
 *参数  :data 
 *BY    :zf
 */
...
/*
 *函数名:LED_WrCmd(uint8 cmd)
 *功能  :写命令函数
 *参数  :无
 *BY    :zf
 */
void LED_WrCmd(uint8 cmd) 
{
	uint8 i = 8;
	//LED_CS = 0;
	LED_DCL;
...
/*
 *函数名:LED_Set_Pos(uint8 x, uint8 y)
 *功能  :设置坐标函数
 *参数  :无
 *BY    :zf
 */
...
void LED_Init(void)        
{
	...
	LED_SCLH;		
	LED_RSTL;                               //从上电到下面开始初始化要有足够的时间，即等待RC复位完毕	
        for(int32 j=0;j<=40000;j++)
        {
          volatile uint8 i ;
          for(i=0;i<1;i++) {
           asm("nop");
...
/*
 *函数名:LED_P6x8Char(uint8 x,uint8 y,uint8 ch)
 *功能  :显示一个6x8标准ASCII字符（即由48个亮点组成）
 *参数  :x为显示的横坐标0~122，y为显示的纵坐标0～7，ch要显示的字符
 *BY    :zf
 */
void LED_P6x8Char(uint8 x,uint8 y,uint8 ch)
{
	...
	for(i=0;i<6;i++)
	{     
		LED_WrDat(F6x8[c][i]);  //ascii码表
	}
}
/*
 *函数名:LED_P6x8Str(byte x,byte y,byte *p)
 *功能  :写入一组6x8标准ASCII字符串
...
/*
 *函数名:LED_Light(byte x,byte y,byte *p)
 *功能  :覆盖一组6x8标准ASCII字符串
 *参数  :x为显示的横坐标0~122，y为页范围0～7，要覆盖的字符串
 *BY    :zf
 */
void LED_Light(uint8 x,uint8 y,uint8 ch[])
{
	uint8 c=0,i=0,j=0;      
	while (ch[j]!='\0')
	{    
 ...
 *功能  :写入一组14x16的中文字符串（字符串表格中需含有此字）
 ...
 *功能  :输出14x16汉字和字符混合字符串 （字符串表格中需含有此字）?
 *参数  :x为显示的横坐标0~114，y为页范围0～3，要显示的中文字符串
 *BY    :zf
 */
...
/*
 *函数名:LED_PrintBMP(byte x0,byte y0,byte x1,byte y1,byte bmp[])
 *功能  :显示BMP图片  大小为(x1-x0+1) × ((y1-y0+1)*8)
...
/*
 *函数名:LED_PrintValue(uchar x, uchar y, char data)
...
/*
 *函数名:void LED_PrintValueI(uchar x, uchar y, int32 data)
 *功能  :将一个int型数转换成5位数进行显示
...
/*
 *函数名: void LED_PrintValueFP(uint8 x, uint8 y, uint32  data, uint8 num)
 *功能  :将长整形不知变为何物
 ...
 *BY    :zf
 */
void LED_PrintValueF(uint8 x, uint8 y, float32 data, uint8 num)
{
...
/*
 *函数名:int16 intabs2(int16 data) 
 *功能  :将负数转化成正数
 *参数  :data 待转化数据
 *BY    :zf
 */
int16 intabs2(int16 data)
{
 ...
 *BY    :zf
 */
uint8 byteabs(int8 data)
{
  return (data>0)?data:-data;
}
/*
 *函数名:uint8 Uint_to_S(uint16 data, int8 *p)
 *功能  :将无符号短整型转化成字符串
 *参数  :uint16 data, int8 *p
 *BY    :zf
 */
...
/*
 *函数名:uint8 Uchar_to_S(uint8 data,int8 *p) 
 *功能  :将一无符号基本整形转化成字符串
 *参数  :uint8 data,int8 *p
 *BY    :zf
 */
uint8 Uchar_to_S(uint8 data,int8 *p) 
{
  uint8 i,j;
  for (i=3;i>0;i--)
  {
...
/*
 *函数名:uint8 Int_to_S(int16 data,int8 *p) 
 *功能  :将有符号短整型数据转化成字符串，有正负号区别
 *参数  :int16 data,int8 *p
 *BY    :zf
 */
uint8 Int_to_S(int16 data,int8 *p)
{
  uint8 i,j;
  if (data<0)
    *p='-';
...
/*
 *函数名:uint8 Int2_to_S(int16 data,int8 *p)
 *功能  :将有符号短整型数据转化成字符串，无正负号区别
 *参数  :int16 data,int8 *p
 *BY    :zf
 */
uint8 Int2_to_S(int16 data,int8 *p)
{
  uint8 i,j;
  data=intabs2(data);
  for(i=5;i>0;i--)
...
/*
 *函数名:uint8 Int_to_S2(int16 data,int8 *p)  
 *功能  :将有符号短整型数据转化成字符串，有正负号区别
 *参数  :无
 *BY    :zf
 */
uint8 Int_to_S2(int16 data,int8 *p) 
{
  uint8 i,j;
  if (data<0) 
    *p='-';
...
/*
 *函数名:uint8 ulong_to_s(uint32 data, int8 *p)
 *功能  :长整形变字符串
 *参数  :uint32 data, int8 *p
 *BY    :zf
 */
uint8 Ulong_to_S(uint32 data, int8 *p)
{ 
   uint8 i,j;
   for (i=10;i>0;i--) 
   {
...
/*
 *函数名:uint8 char_to_s(int8 data,int8 *p) 
 *功能  :有符号基本整形转化成字符串
 *参数  :data 待转化基本整形  
 *BY    :zf
 */
uint8 Char_to_S(int8 data,int8 *p) 
{
  uint8 i,j;
  if (data<0) 
  {
...
/*
 *函数名:void LED_Cursor(uint8 cursor_column, uint8 cursor_row)
 *功能  :显示一个方块
 *参数  :x为显示的横坐标0~122，y为页范围0～7，ch要显示的字符
 *BY    :zf
 */
...
{	
 	...
 	{
 		if(cursor_column == 1) 
                  LED_Set_Pos(0, cursor_row + 2);
 		else  
                  LED_Set_Pos(80 + (cursor_column - 2)*6, cursor_row + 2);
 		LED_WrDat(0xFF);
					 
 	}
}
/*
 *函数名:void LED_INT_PRINT(uint8 x, uint8 y, uint8 ch[], int32 data)
 *功能  :赋名整形输出
 *参数  :x为显示的横坐标0~122，y为页范围0～7，ch要显示的字符
 *BY    :
 */
...
/*
 *函数名:void LED_FLOAT_PRINT(uint8 x, uint8 y, uint8 ch[], int32 data)
 *功能  :赋名浮点输出
 *参数  :x为显示的横坐标0~122，y为页范围0～7，ch要显示的字符
 *BY    :
 */
