...
void adc_init(ADCn adcn, ADC_Ch ch)
{
    ...
    case ADC0:       /*   ADC0  */
        ...
        SIM_SOPT7 &= ~(SIM_SOPT7_ADC0ALTTRGEN_MASK  | SIM_SOPT7_ADC0PRETRGSEL_MASK);//使用PDB触发 触发源A
        SIM_SOPT7 = SIM_SOPT7_ADC0TRGSEL(0);//PDB外部引脚触发输入

        switch(ch)
        {
        case AD8:   //ADC0_SE8 -- PTB0
        case AD9:   //ADC0_SE9 -- PTB1
            SIM_SCGC5 |= SIM_SCGC5_PORTB_MASK;//开启引脚时钟
            PORT_PCR_REG(PORTB_BASE_PTR, ch - AD8 + 0) =  PORT_PCR_MUX(0);//设置引脚的模式模拟输入
            break;
        case AD10:  //ADC0_SE10 -- PTA7
        case AD11:  //ADC0_SE11 -- PTA8
            SIM_SCGC5 |= SIM_SCGC5_PORTA_MASK;//开启引脚时钟
            PORT_PCR_REG(PORTA_BASE_PTR, ch - AD10 + 7) =  PORT_PCR_MUX(0);//设置引脚为模式0模拟输入
            break;
        case AD12:  //ADC0_SE12 -- PTB2
        case AD13:  //ADC0_SE13 -- PTB3
            SIM_SCGC5 |= SIM_SCGC5_PORTB_MASK;//开启引脚时钟
            PORT_PCR_REG(PORTB_BASE_PTR, ch - AD12 + 2) =  PORT_PCR_MUX(0);//设置模拟输入模式
            break;
        case AD14:  //ADC0_SE14 -- PTC0
        case AD15:  //ADC0_SE15 -- PTC1
            SIM_SCGC5 |= SIM_SCGC5_PORTC_MASK;//开启引脚时钟
            PORT_PCR_REG(PORTC_BASE_PTR, ch - AD14 + 0) =  PORT_PCR_MUX(0);//设置模拟输入模式
            break;
        case AD17:   //ADC0_SE17 -- PTE24
        case AD18:   //ADC0_SE17 -- PTE25
            SIM_SCGC5 |= SIM_SCGC5_PORTE_MASK;//开启引脚时钟
            PORT_PCR_REG(PORTE_BASE_PTR, ch - AD17 + 24) =  PORT_PCR_MUX(0);//设置模拟输入模式
            break;
        default:
            return;
        }
...
u16 ad_once(ADCn adcn, ADC_Ch ch, ADC_nbit bit) //采集某路模拟量的AD值
{
    ...
    result = ADC_R_REG(ADCx[adcn], 0);//获取结果
    ADC_SC1_REG(ADCx[adcn], 0) &= ~ADC_SC1_COCO_MASK;//清除转换完成标志位
    return result;
}
...
void adc_start(ADCn adcn, ADC_Ch ch, ADC_nbit bit)
{
    ...
    //初始化ADC默认配置   正常电源模式 总线时钟 4分频 长时间采样使能  设置精度
    Master_Adc_Config.CONFIG1  = ADLPC_NORMAL
                                 | ADC_CFG1_ADIV(ADIV_4)
                                 | ADLSMP_LONG
                                 | ADC_CFG1_MODE(bit)
                                 | ADC_CFG1_ADICLK(ADICLK_BUS);
    Master_Adc_Config.CONFIG2  = MUXSEL_ADCA    //MUXSEL_ADCA  配置禁止异步时钟输出使能 ADxxat通道的选择 高速配置 长采样时间
                                 | ADACKEN_DISABLED
                                 | ADHSC_HISPEED
