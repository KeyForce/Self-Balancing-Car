...
*            duty        占空比
*  函数返回：无
*  修改时间：2012-2-14   注意通道的输出使能   CHxOM为缺省0的时候 
...
void FTM_PWM_init(FTMn ftmn, CHn ch, u32 freq, u32 duty)
{
    ...
    /******************* 开启时钟 和 复用IO口*******************/
    //注，这里代码虽然长，但编译时会删掉很多没用的部分，不影响速度  SCGC6时钟门控制寄存器
    switch(ftmn)
    {
    case FTM0:
        ...
        case CH0:
            if(FTM0_CH0 == PTC1)
            {
                SIM_SCGC5 |= SIM_SCGC5_PORTC_MASK;//使能PORTC引脚的时钟
                PORT_PCR_REG(PORTC_BASE_PTR, 1) = PORT_PCR_MUX(4);  // PTC1 选择复用功能4
            }
            else if(FTM0_CH0 == PTA3)//这两者属于映射关系
            {
                SIM_SCGC5 |= SIM_SCGC5_PORTA_MASK;//使能PORTA时钟
                PORT_PCR_REG(PORTA_BASE_PTR, 3) = PORT_PCR_MUX(3);  // PTA3 选择复用功能3
            }
            else
            {
    ...
    case FTM1:
        ...
        case CH0:
            if(FTM1_CH0 == PTA8)
            {
                SIM_SCGC5 |= SIM_SCGC5_PORTA_MASK;//使能PORTA模块的时钟
    ...
    //MSnB:MSnA = 1x      
    //ELSnB:ELSnA=01    上升沿(自已注)
    // ELSnB:ELSnA = 10   High-true pulses  (clear Output on match)下降沿
    // ELSnB:ELSnA = 11   Low-true pulses   (set Output on match)跳变沿
    // Edge-aligned PWM  边沿对齐PWM波   《k16 reference manual.pdf》  P944  或者  《K60P144M100SF2RM.pdf》P1001


    /******************** 配置时钟和分频 ********************/
    //                        设置PWM模式中央对齐模式     分频                  时钟选择
    FTM_SC_REG(FTMx[ftmn])    = FTM_SC_CPWMS_MASK | FTM_SC_PS(sc_ps) | FTM_SC_CLKS(1);  //没有使能溢出中断
    FTM_CNTIN_REG(FTMx[ftmn]) = 0;//装入初始值                                                      // Channel (n) Value  。设置脉冲宽度：(CnV - CNTIN).
    FTM_MOD_REG(FTMx[ftmn])   = mod;//设置计数器的周期                                                    //Modulo value模数, EPWM的周期为 ：MOD - CNTIN + 0x0001
    FTM_CNTIN_REG(FTMx[ftmn]) = 0;//计数器的初始值                                                      //Counter Initial Value 计数器初始化值
    FTM_CnV_REG(FTMx[ftmn], ch) = cv;//因为是PWM模式所以是输出比较寄存器
    FTM_CNT_REG(FTMx[ftmn])   = 0;//通过向这个寄存器中写入任何一个值,则会将CNTIN的值装载到FTM计数器中                                                      //计数器。只有低16位可用
...
void FTM_PWM_Duty(FTMn ftmn, CHn ch, u32 duty)
{
    ...
    // 配置FTM通道值
    FTM_CnV_REG(FTMx[ftmn], ch) = cv;//设置CnV比较寄存器
...
void FTM_Input_init(FTMn ftmn, CHn ch, Input_cfg cfg)
{
    ...
    switch(cfg)
    {
    case Rising:    //上升沿触发    MSB MSA00捕获模式   ELSB ELSA01 上升沿     中断使能
    ...
    case Falling:   //下降沿触发     ELSB ELSA 10下降沿
    ...
    case Rising_or_Falling: //上升沿、下降沿都触发   ELSB ELSA 11跳变沿
    ...
    FTM_MODE_REG(FTMx[ftmn]) |= FTM_MODE_WPDIS_MASK;//去除写保护
    FTM_COMBINE_REG(FTMx[ftmn]) = 0;//去除联合PWM状态
...
void FTM_QUAD_Init1(FTMn ftmn, CHn ch)
{
   
  	ASSERT(  ( (ftmn == FTM1 || ftmn == FTM2 ) && (ch <= CH1)) );
	switch(ftmn)
	{
	case FTM1 :
		SIM_SCGC6 |= SIM_SCGC6_FTM1_MASK;//使能FTM1时钟
			...
			PORT_PCR_REG(PORTA_BASE_PTR, 13) = PORT_PCR_MUX(7);  // PTA12
          	  	}
           	 	else if(FTM1_CH0 == PTB0)
           		 {
                	SIM_SCGC5 |= SIM_SCGC5_PORTB_MASK;
                	PORT_PCR_REG(PORTB_BASE_PTR, 0) = PORT_PCR_MUX(6);  // PTB0
			PORT_PCR_REG(PORTB_BASE_PTR, 1) = PORT_PCR_MUX(6);  // PTB0
            		 }
            		else
            		{
                	assert_failed(__FILE__, __LINE__);      //设置管脚有误？
		...
		SIM_SCGC3|=SIM_SCGC3_FTM2_MASK;//使能FTM2时钟 
	 	switch(ch)
	  	...
	  	case CH0:
			...
			PORT_PCR_REG(PORTA_BASE_PTR, 11) = PORT_PCR_MUX(6);  // PTA10
           		 }
           	 	else if(FTM2_CH0 == PTB18)
           	 	{
               	 	SIM_SCGC5 |= SIM_SCGC5_PORTB_MASK;
                	PORT_PCR_REG(PORTA_BASE_PTR, 18) = PORT_PCR_MUX(3);  // PTB18
			PORT_PCR_REG(PORTA_BASE_PTR, 19) = PORT_PCR_MUX(3);  // PTB18
            		}
           	 	else
           	 	{
                	assert_failed(__FILE__, __LINE__);                   //设置管脚有误？
        	...
        	case CH1:
            		...
            		{
                	SIM_SCGC5 |= SIM_SCGC5_PORTA_MASK;
			PORT_PCR_REG(PORTA_BASE_PTR, 10) = PORT_PCR_MUX(6);  // PTA11
                	PORT_PCR_REG(PORTA_BASE_PTR, 11) = PORT_PCR_MUX(6);  // PTA11
           	 	}
          	  	else if(FTM2_CH1 == PTB19)
           	 	{
                	SIM_SCGC5 |= SIM_SCGC5_PORTB_MASK;
			PORT_PCR_REG(PORTA_BASE_PTR, 18) = PORT_PCR_MUX(6);  // PTB19
	...
	FTM_QDCTRL_REG(FTMx[ftmn])|=FTM_QDCTRL_QUADEN_MASK;//启用FTM1正交解码模式  
   
	FTM_MODE_REG(FTMx[ftmn])|= FTM_MODE_FTMEN_MASK;//FTM1EN=1    
      ...
      switch(ch)
	{
        case CH0:
	    {
	      ...
	      PORTA_PCR9= PORT_PCR_MUX(6); // 设置引脚A9引脚为FTM1_PHB功能  
	       SIM_SCGC6|=SIM_SCGC6_FTM1_MASK;  //使能时钟
		FTM1_MODE |= FTM_MODE_WPDIS_MASK;//写保护禁止
		FTM1_QDCTRL|=FTM_QDCTRL_PHAFLTREN_MASK;//滤波使能
		FTM1_QDCTRL|=FTM_QDCTRL_PHBFLTREN_MASK;//滤波使能
		FTM1_FILTER=FTM_FILTER_CH0FVAL(14);
		FTM1_FILTER=FTM_FILTER_CH1FVAL(14);
		PORT_PCR_REG(PORTA_BASE_PTR, 8) |= PORT_PCR_PE_MASK | PORT_PCR_PS_MASK ;     //开弱上
        ...
        case CH2:
	      {
	         PORTB_PCR0= PORT_PCR_MUX(6); // 设置引脚B0引脚为FTM2_PHA功能  
		 PORTB_PCR1= PORT_PCR_MUX(6); // 设置引脚B1引脚为FTM2_PHB功能  
		SIM_SCGC6|=SIM_SCGC6_FTM1_MASK;  //使能时钟
		FTM1_MODE |= FTM_MODE_WPDIS_MASK;//写保护禁止
                FTM1_QDCTRL|=FTM_QDCTRL_PHAFLTREN_MASK;//滤波使能
		FTM1_QDCTRL|=FTM_QDCTRL_PHBFLTREN_MASK;//滤波使能
   ...
   switch(ch)
	{
        case CH0:
	   {
		...
		FTM2_QDCTRL|=FTM_QDCTRL_PHAFLTREN_MASK;//滤波使能
		FTM2_QDCTRL|=FTM_QDCTRL_PHBFLTREN_MASK;//滤波使能
		FTM2_FILTER=FTM_FILTER_CH0FVAL(14);
		FTM2_FILTER=FTM_FILTER_CH1FVAL(14);
        ...
        case CH1:
	   {
        ...
        default:printf("Data error!2");  
        }
        break;
    }
   break;
   default:printf("Data error!3");  
   }         
}
