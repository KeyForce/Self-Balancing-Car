...
void lptmr_counter_init(LPT0_ALTn altn, u16 count, u8 PrescaleValue, LPT_CFG cfg)
{
    ...
    /* 设置累加计数值  */
    LPTMR_CMR_REG(LPTMR0_BASE_PTR)  =   LPTMR_CMR_COMPARE(count);//设置比较值                       //设置比较值
    ...
    //                                     选择LPO时钟               开启和配置脉冲滤波器：2^n个时钟上升沿才识别
    //                                                 貌似这里配置有问题虽然配置了分频器滤波器，但是配置了分频滤波绕道，也就是说分频滤波被禁止。此时单脉冲工作模式。                                        
    /* 管脚设置、使能中断 控制状态寄存器 */
    ...
    //                                       选择输入管脚        选择脉冲计数                                   下降沿       上升沿           使能LPT              中断使能
    // TFC = 0，即计数值等于比较值时，计数值复位

    enable_irq(LPTMR_irq);	                //开引脚的嵌套向量IRQ中断
}
/*
//清空计数
...
*  函数返回：无
*  修改时间：2012-1-20
*  备    注：官方例程自带  使能的是时间计数模式
*************************************************************************/
void time_delay_ms(uint32 ms)//
{
    /* Make sure the clock to the LPTMR is enabled */
    SIM_SCGC5 |= SIM_SCGC5_LPTIMER_MASK;//使能LPTMR的模块时钟

    /* Set the compare value to the number of ms to delay */
    LPTMR0_CMR = ms;//设置比较值就是周期

    /* Set up LPTMR to use 1kHz LPO with no prescaler as its clock source */
    LPTMR0_PSR = LPTMR_PSR_PCS(1) | LPTMR_PSR_PBYP_MASK;
//时钟选择LPO  PCS1-0位,PBYP位分频器绕道,预分频6-3都是0
    /* Start the timer */
    LPTMR0_CSR = LPTMR_CSR_TEN_MASK;//使能定时器(模式比较标志位1计时器复位)参考寄存器CSR
//禁能中断
    /* Wait for counter to reach compare value */
    while (!(LPTMR0_CSR & LPTMR_CSR_TCF_MASK));//
//等待标志位为1
    /* Clear Timer Compare Flag */
    LPTMR0_CSR &= ~LPTMR_CSR_TEN_MASK;//软件清除标志位
//所有设置只有在禁能定时器之前有效
    return;
}
